...
// Tone options for emails
export type EmailTone = 'professional' | 'friendly' | 'formal' | 'concise' | 'casual';
...
interface RewriteParams {
  text: string;
  style: 'formal' | 'friendly' | 'concise' | 'expand' | 'fix-grammar';
}
...
class AIService {
   ...
   */
  async getSmartComposeSuggestion(params: SmartComposeParams): Promise<string | null> {
    ...
    if (currentText.length < 10) return null;
...
If the text seems complete, respond with empty string.`;
   ...
   */
  async generateSmartReplies(params: SmartReplyParams): Promise<string[]> {
...
["Reply 1 text", "Reply 2 text", "Reply 3 text"]`;
      ...
      try {
        ...
        return lines.slice(0, 3).map((l: string) => l.replace(/^[\d\.\-\*\)]+\s*/, '').replace(/^["']|["']$/g, ''));
      }
   ...
   */
  async summarizeEmail(params: SummarizeParams): Promise<{
    summary: string;
    keyPoints: string[];
    actionItems: string[];
  } | null> {
...
Return ONLY valid JSON, no explanations.`;
   ...
   */
  async rewriteText(params: RewriteParams): Promise<string | null> {
    if (!this.isConfigured) return null;
...
Return ONLY the rewritten text, no explanations or quotes.`;
   ...
   */
  async suggestSubjectLines(emailBody: string, count: number = 3): Promise<string[]> {
    if (!this.isConfigured) return [];
...
Return ONLY valid JSON, no explanations.`;
   ...
   */
  async extractKeyInfo(emailContent: string): Promise<{
    dates: string[];
    people: string[];
    topics: string[];
