...
class EmailService {
  ...
  getStatus(): { configured: boolean; fromAddress: string; fromName: string } {
    return {
  ...
  updateConfig(config: Partial<EmailConfig & { fromAddress?: string; fromName?: string }>): void {
    if (config.fromAddress) this.fromAddress = config.fromAddress;
