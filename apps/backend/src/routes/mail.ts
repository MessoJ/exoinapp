...
import { snoozeService, SNOOZE_PRESETS } from '../services/snoozeService';
...
function calculatePresetTime(preset: string, now: Date): { iso: string; display: string } {
  let time: Date;
  let display: string;
...
interface ParsedSearchQuery {
  ...
  attachmentType: string[]; // has:pdf, has:doc, etc.
  isStarred: boolean | null;
  isRead: boolean | null;
  labels: string[];         // label: operator values
  folder: string | null;    // in: or folder: operator
  before: Date | null;      // before: operator
  after: Date | null;       // after: operator
  olderThan: string | null; // older_than: (e.g., "7d", "1m")
  newerThan: string | null; // newer_than: 
    ...
    if (operator) {
      ...
      switch (operator) {
        ...
        case 'has':
          ...
          break;
        case 'is':
          if (value === 'starred') result.isStarred = true;
          else if (value === 'unstarred') result.isStarred = false;
          else if (value === 'read') result.isRead = true;
          else if (value === 'unread') result.isRead = false;
          break;
        case 'label':
...
function buildSearchWhere(userId: string, parsed: ParsedSearchQuery): Prisma.EmailWhereInput {
  const conditions: Prisma.EmailWhereInput[] = [{ userId }];
  ...
  if (parsed.text.length > 0) {
    const textConditions = parsed.text.map(term => ({
      OR: [
        ...
        { textBody: { contains: term, mode: 'insensitive' as const } },
    ...
    conditions.push({
      OR: parsed.from.map(addr => ({
        OR: [
          ...
          { fromName: { contains: addr, mode: 'insensitive' as const } },
        ]
      }))
    });
  }
      ...
      attachments: {
        some: {
          OR: parsed.attachmentType.map(type => {
            ...
            return { 
              OR: [
                { filename: { endsWith: `.${type}`, mode: 'insensitive' as const } },
                { mimeType: { contains: type, mode: 'insensitive' as const } }
              ]
            };
          })
        }
  ...
  if (parsed.folder) {
    ...
    if (validFolders.includes(parsed.folder)) {
      conditions.push({ folder: parsed.folder as EmailFolder });
    }
  }
...
export default async function mailRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/account/setup', {
    ...
    const { id: userId, email: userEmail } = (request as any).user;
    ...
    const result = await mailSyncService.saveMailCredentials(userId, userEmail, password);
    
    if (result.success) {
      // Do initial sync
      const syncResult = await mailSyncService.syncAllFolders(userId, userEmail, password);
      return { 
        ...result, 
  ...
  fastify.post('/account/test', {
    ...
    const { email: userEmail } = (request as any).user;
    const { password } = request.body as any;
    ...
    const result = await mailSyncService.testConnection(userEmail, password);
    return result;
  });
  ...
  fastify.get('/account/status', {
    ...
    const creds = await mailSyncService.getMailCredentials(userId);
  ...
  fastify.post('/sync/quick', {
    ...
    const result = await mailSyncService.quickSync(userId);
  ...
  fastify.post('/sync/full', {
    ...
    let creds = await mailSyncService.getMailCredentials(userId);
    ...
    if (password && !creds) {
      await mailSyncService.saveMailCredentials(userId, userEmail, password);
    }

    const result = await mailSyncService.syncAllFolders(userId, emailToUse, passToUse);
  ...
  fastify.get('/sync/check', {
    ...
    const result = await mailSyncService.checkNewMail(userId);
    return result;
  });
  ...
  fastify.get('/folders', {
    ...
    const folderCounts = await Promise.all([
      prisma.email.count({ where: { userId, folder: 'INBOX' } }),
      prisma.email.count({ where: { userId, folder: 'SENT' } }),
      prisma.email.count({ where: { userId, folder: 'DRAFTS' } }),
      prisma.email.count({ where: { userId, folder: 'TRASH' } }),
      prisma.email.count({ where: { userId, folder: 'SPAM' } }),
      prisma.email.count({ where: { userId, folder: 'ARCHIVE' } }),
      prisma.email.count({ where: { userId, folder: 'INBOX', isRead: false } }),
      prisma.email.count({ where: { userId, folder: 'SPAM', isRead: false } }),
    ]);
  ...
  fastify.get('/messages', {
    ...
    const { folder = 'INBOX', page = '1', limit = '25', search } = request.query as any;
    const { id: userId } = (request as any).user;
  ...
  fastify.get('/history/:email', {
    ...
    const { email: contactEmail } = request.params as { email: string };
    const { id: userId } = (request as any).user;
    ...
    return {
      ...
      emails: emails.map(e => ({
        ...
        to: e.toAddresses.map((addr: string) => ({ address: addr })),
  ...
  fastify.get('/messages/:id', {
    ...
    if (!email.isRead) {
      ...
      mailSyncService.markAsRead(userId, email.messageId, true).catch(console.error);
    }
  ...
  fastify.get('/thread/:subjectQuery', {
    ...
    const { subjectQuery } = request.params as { subjectQuery: string };
    const { id: userId } = (request as any).user;
  ...
  fastify.get('/attachments/:attachmentId', {
    ...
    try {
      const content = await mailSyncService.fetchAttachment(userId, attachment.email.messageId, attachment.id);
      if (content) {
  ...
  fastify.post('/send', {
    ...
    const { id: userId, email: userEmail } = (request as any).user;
    const { to, cc, bcc, subject, html, text, includeSignature, priority = 'normal', replyTo, forwardOf, attachDocumentId, attachments: reqAttachments } = request.body as any;
    ...
    const messageId = `<${uuidv4()}@exoinafrica.com>`;
  ...
  fastify.post('/send/queue', {
    ...
    const { 
      ...
      includeSignature, priority = 'normal',
      undoDelaySeconds = 10, // Default 10 seconds
    ...
    // Queue the email
    const result = await outboxService.queueEmail({
      userId,
      fromName: `${sender.firstName} ${sender.lastName}`,
      fromAddress: userEmail,
      to: toAddresses,
  ...
  fastify.post('/send/undo/:outboxId', {
    ...
    const result = await outboxService.cancelEmail(outboxId, userId);
  ...
  fastify.get('/outbox', {
    ...
    const pending = await outboxService.getPendingEmails(userId);
    ...
    return {
      emails: pending.map(email => ({
        ...
        status: email.status,
        timeRemaining: outboxService.getTimeRemaining(email.sendAt),
  ...
  fastify.get('/scheduled', {
    ...
    const scheduled = await outboxService.getScheduledEmails(userId);
  ...
  fastify.put('/scheduled/:outboxId', {
    ...
    const { outboxId } = request.params as { outboxId: string };
    const { id: userId } = (request as any).user;
    const { to, cc, bcc, subject, html, text, scheduledAt } = request.body as any;

    const result = await outboxService.updateScheduledEmail(outboxId, userId, {
      to, cc, bcc, subject, html, text,
      scheduledAt: scheduledAt ? new Date(scheduledAt) : undefined,
    });
  ...
  fastify.put('/messages/:id/read', {
    ...
    const { id } = request.params as any;
    const { isRead } = request.body as any;
    const { id: userId } = (request as any).user;
    ...
    // Sync to server
    mailSyncService.markAsRead(userId, email.messageId, isRead).catch(console.error);
  ...
  fastify.put('/messages/:id/star', {
    ...
    const { id } = request.params as any;
    const { isStarred } = request.body as any;
    const { id: userId } = (request as any).user;
  ...
  fastify.put('/messages/:id/move', {
    ...
    const { id } = request.params as any;
    const { folder } = request.body as any;
    const { id: userId } = (request as any).user;
    ...
    await prisma.email.update({
      where: { id },
      data: { folder: folder as EmailFolder }
    });

    // Sync to server
    mailSyncService.moveEmail(userId, email.messageId, folder).catch(console.error);
  ...
  fastify.delete('/messages/:id', {
    ...
    if (email.folder === 'TRASH') {
      ...
      mailSyncService.deleteEmail(userId, email.messageId).catch(console.error);
  ...
  fastify.post('/messages/bulk', {
    ...
    const { ids, action, folder } = request.body as any;
    const { id: userId } = (request as any).user;
  ...
  fastify.get('/search', {
    ...
    const { q, folder } = request.query as any;
    const { id: userId } = (request as any).user;
    ...
    if (folder && folder !== 'ALL') {
      where.folder = folder as EmailFolder;
    }
  ...
  fastify.get('/search/fulltext', {
    ...
    const { q, folder, page = '1', limit = '50' } = request.query as any;
    const { id: userId } = (request as any).user;
    ...
    const tsQuery = q
      ...
      .map((word: string) => `${word}:*`)  // Add prefix matching
      .join(' & ');
    ...
    const emails: any[] = await prisma.$queryRawUnsafe(`
      SELECT 
    ...
    const results = emails.map(email => ({
      ...
      subject: email.subject,
      from: { name: email.fromName, address: email.fromAddress },
      to: email.toAddresses?.map((addr: string) => ({ address: addr })) || [],
      date: email.sentAt?.toISOString(),
      snippet: email.snippet,
  ...
  fastify.get('/search/advanced', {
    ...
    const { q, page = '1', limit = '50' } = request.query as any;
    const { id: userId } = (request as any).user;
  ...
  fastify.get('/search/suggestions', {
    ...
    const senderSuggestions = await prisma.email.findMany({
      where: {
        userId,
        OR: [
          { fromAddress: { contains: q, mode: 'insensitive' } },
    ...
    // Get subject matches
    const subjectSuggestions = await prisma.email.findMany({
      where: {
        userId,
        subject: { contains: q, mode: 'insensitive' }
      },
    ...
    // Get label matches
    const labelSuggestions = await prisma.email.findMany({
      where: {
        userId,
        labels: { hasSome: [q] }
      },
  ...
  fastify.put('/messages/:id/labels', {
    ...
    const { id } = request.params as any;
    const { label, action } = request.body as any;
    const { id: userId } = (request as any).user;
  ...
  fastify.post('/messages/:id/snooze', {
    ...
    const { preset, customTime } = request.body as { preset?: string; customTime?: string };
    const { id: userId } = (request as any).user;

    const result = await snoozeService.snoozeEmail({
      emailId: id,
      userId,
      preset: preset as keyof typeof SNOOZE_PRESETS,
    ...
    return {
      ...
      timeRemaining: snoozeService.getTimeRemaining(result.snoozedUntil),
    };
  });
  ...
  fastify.delete('/messages/:id/snooze', {
    ...
    const result = await snoozeService.unsnoozeEmail(id, userId);
  ...
  fastify.put('/messages/:id/snooze', {
    ...
    const { id } = request.params as { id: string };
    const { newTime } = request.body as { newTime: string };
    const { id: userId } = (request as any).user;
    ...
    const result = await snoozeService.updateSnoozeTime(id, userId, new Date(newTime));
  ...
  fastify.get('/snoozed', {
    ...
    const emails = await snoozeService.getSnoozedEmails(userId);
    ...
    return {
      emails: emails.map(email => ({
        ...
        timeRemaining: email.snoozedUntil ? snoozeService.getTimeRemaining(email.snoozedUntil) : null,
        hasAttachments: email.hasAttachments,
        attachments: email.attachments,
      })),
    };
  ...
  fastify.get('/inbox/priority', {
    ...
    const result = await priorityService.getPrioritizedInbox(userId, parseInt(limit));

    // Transform emails for response
    const transformEmail = (email: any) => ({
      id: email.id,
      messageId: email.messageId,
      subject: email.subject,
      from: { name: email.fromName, address: email.fromAddress },
  ...
  fastify.get('/inbox/priority/counts', {
    ...
    const { id: userId } = (request as any).user;
    const counts = await priorityService.getSmartInboxCounts(userId);
    return { counts };
  });
  ...
  fastify.get('/messages/:id/priority', {
    ...
    const insight = await priorityService.getEmailPriorityInsight(id, userId);
  ...
  fastify.post('/vacation-responder', {
    ...
    const existing = await prisma.vacationResponder.findUnique({
      where: { userId }
    });
    ...
    if (existing) {
      // Update existing
      responder = await prisma.vacationResponder.update({
        where: { userId },
        data: {
      ...
      // Create new
      responder = await prisma.vacationResponder.create({
        data: {
          userId,
          startDate: new Date(startDate),
          endDate: new Date(endDate),
  ...
  fastify.delete('/vacation-responder', {
    ...
    await prisma.vacationResponder.deleteMany({
      where: { userId }
    });
...
async function generateSignature(user: any, company: any): Promise<string> {
