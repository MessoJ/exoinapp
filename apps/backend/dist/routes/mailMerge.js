...
exports.default = mailMergeRoutes;
const mailMergeService_1 = require("../services/mailMergeService");
async function mailMergeRoutes(fastify) {
    ...
    fastify.post('/parse-csv', async (request, reply) => {
        ...
        const recipients = mailMergeService_1.mailMergeService.parseCSV(csvContent);
        const sampleFields = mailMergeService_1.mailMergeService.getSampleMergeFields(recipients);
        return reply.send({
            recipients,
    ...
    fastify.post('/extract-fields', async (request, reply) => {
        const { content } = request.body;
        if (!content) {
            return reply.status(400).send({ error: 'Content is required' });
        }
        const fields = mailMergeService_1.mailMergeService.extractMergeFields(content);
    ...
    fastify.post('/validate', async (request, reply) => {
        ...
        const validation = mailMergeService_1.mailMergeService.validateRecipients(recipients, requiredFields);
    ...
    fastify.post('/preview', async (request, reply) => {
        ...
        const preview = mailMergeService_1.mailMergeService.previewMailMerge(subject, content, recipient);
        return reply.send(preview);
    });
    // Create new mail merge campaign
    fastify.post('/', async (request, reply) => {
        const userId = request.user.id;
        const { name, subject, htmlBody, recipients, scheduledAt, } = request.body;
        if (!name || !subject || !htmlBody || !recipients) {
            return reply.status(400).send({
                error: 'Name, subject, htmlBody, and recipients are required',
            });
        }
        const mailMerge = await mailMergeService_1.mailMergeService.createMailMerge({
            name,
            subject,
            htmlBody,
            recipients,
    ...
    fastify.get('/', async (request, reply) => {
        ...
        const result = await mailMergeService_1.mailMergeService.getUserMailMerges(userId, page, limit);
    ...
    fastify.get('/:id', async (request, reply) => {
        ...
        const mailMerge = await mailMergeService_1.mailMergeService.getMailMerge(id, userId);
        if (!mailMerge) {
    ...
    fastify.post('/:id/start', async (request, reply) => {
        const userId = request.user.id;
        const { id } = request.params;
        const success = await mailMergeService_1.mailMergeService.startMailMerge(id, userId);
        if (!success) {
    ...
    fastify.post('/:id/cancel', async (request, reply) => {
        const userId = request.user.id;
        const { id } = request.params;
        const success = await mailMergeService_1.mailMergeService.cancelMailMerge(id, userId);
        if (!success) {
    ...
    fastify.delete('/:id', async (request, reply) => {
        const userId = request.user.id;
        const { id } = request.params;
        const success = await mailMergeService_1.mailMergeService.deleteMailMerge(id, userId);
        if (!success) {
