...
class PriorityService {
     ...
     */
    async calculatePriorityScore(email, userId) {
        let score = 0;
        const factors = [];
        ...
        const isDirectRecipient = email.toAddresses.some(addr => userEmails.some(ue => addr.toLowerCase().includes(ue.toLowerCase())));
        ...
        const isAutomated = AUTOMATED_PATTERNS.some(pattern => pattern.test(email.fromAddress));
        if (isAutomated) {
     ...
     */
    async getSenderStats(userId, senderAddress) {
        // Count emails received from this sender
        const receivedFrom = await index_1.prisma.email.count({
            where: {
                userId,
        ...
        const repliedTo = await index_1.prisma.email.count({
            where: {
                userId,
                folder: 'SENT',
        ...
        const lastEmail = await index_1.prisma.email.findFirst({
            where: {
                userId,
                OR: [
                    { fromAddress: { contains: senderAddress, mode: 'insensitive' } },
     ...
     */
    async getPrioritizedInbox(userId, limit = 100) {
        ...
        const importantAndUnread = scoredEmails
            .filter(({ email, score }) => !email.isStarred &&
            !email.isRead &&
     ...
     */
    async getEmailPriorityInsight(emailId, userId) {
        ...
        const reasons = priorityScore.factors
            ...
            .map(f => reasonMap[f] || f);
        const level = priorityScore.score >= 25 ? 'high' :
     ...
     */
    async getSmartInboxCounts(userId) {
        // Get starred count
        const starred = await index_1.prisma.email.count({
            where: { userId, folder: 'INBOX', isStarred: true, snoozedUntil: null }
        });
        // Get total inbox
        const total = await index_1.prisma.email.count({
            where: { userId, folder: 'INBOX', snoozedUntil: null }
        });
        // Get all unread emails for importance check
        const unreadEmails = await index_1.prisma.email.findMany({
            where: {
                userId,
                folder: 'INBOX',
                isRead: false,
        ...
        for (const email of unreadEmails) {
            ...
            const isDirect = email.toAddresses.some(addr => userEmails.some(ue => addr.toLowerCase().includes(ue.toLowerCase())));
