...
exports.snoozeService = exports.SNOOZE_PRESETS = void 0;
const index_1 = require("../index");
// Snooze preset options
exports.SNOOZE_PRESETS = {
...
class SnoozeService {
    ...
    async snoozeEmail(params) {
        ...
        await index_1.prisma.email.update({
            ...
            data: {
                ...
                folder: 'SNOOZED', // Move to snoozed "folder"
            },
        });
    ...
    async unsnoozeEmail(emailId, userId) {
        const email = await index_1.prisma.email.findFirst({
    ...
    // Update snooze time
    async updateSnoozeTime(emailId, userId, newTime) {
        const email = await index_1.prisma.email.findFirst({
            where: { id: emailId, userId, snoozedUntil: { not: null } },
        });
        if (!email) {
    ...
    async getSnoozedEmails(userId) {
        return index_1.prisma.email.findMany({
            where: {
                userId,
    ...
    async processSnooze() {
        ...
        const readyEmails = await index_1.prisma.email.findMany({
            where: {
...
exports.snoozeService = new SnoozeService();
