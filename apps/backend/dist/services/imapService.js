...
class ImapService {
    ...
    createClient(mailboxEmail, mailboxPassword) {
    ...
    // Get folder list
    async getFolders(email, password) {
        const client = this.createClient(email, password);
        if (!client)
            return [];
        try {
    ...
    parseAddressList(addrs) {
        ...
        if (addrs.value && Array.isArray(addrs.value)) {
            return addrs.value.map((a) => this.parseAddress(a)).filter((a) => a.address);
        }
        return [this.parseAddress(addrs)].filter(a => a.address);
    }
    // Sync folder for a user
    async syncFolder(userId, userEmail, userPassword, folderName = 'INBOX', limit = 50) {
        ...
        try {
            ...
            for await (const message of client.fetch(`${startSeq}:*`, {
                ...
                try {
                    ...
                    const hasAttachments = message.bodyStructure?.childNodes?.some((node) => node.disposition === 'attachment') || false;
                    const attachments = [];
                        ...
                        for (const node of bodyStructure.childNodes) {
                            if (node.disposition === 'attachment' ||
                                ...
                                attachments.push({
                                    ...
                                    mimeType: node.type ? `${node.type}/${node.subtype || ''}` : 'application/octet-stream',
                                    size: node.size || 0,
                                });
                            }
                        }
    ...
    async syncAllFolders(userId, userEmail, userPassword) {
