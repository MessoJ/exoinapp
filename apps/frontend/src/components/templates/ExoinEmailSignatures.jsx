...
import { Phone, Mail, Globe, MapPin, Moon, Sun, Save, Check } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import api from '../../lib/api';

// --- REUSABLE LOGO COMPONENT (Adaptive with Tighter Split) ---
export const SigLogo = ({ variant = 'light', scale = 1 }) => {
...
export const EmailSignatureTemplate = ({ data, darkMode }) => {
    const {
        firstName,
        lastName,
        jobTitle,
    ...
    const displayTitle = jobTitle || 'Chief Operations Officer';
    const displayPhone = phone || '+254 700 000 000';
    const displayEmail = email || 'john@exoin.africa';
    const displayLocation = location || 'Nairobi, Kenya';
    const displayWebsite = company?.website || 'www.exoin.africa';
    const displayAddress = officeAddress || 'Exoin Tower, Westlands Road';
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
                ...
                <div className={`w-16 h-16 rounded-full border-2 shadow-md overflow-hidden flex-shrink-0 relative ${
                    darkMode ? 'bg-slate-800 border-slate-600' : 'bg-slate-200 border-white'
                     ...
                     <div className="absolute inset-0 flex items-center justify-center text-[8px] font-bold opacity-50">
                        {firstName ? `${firstName.charAt(0)}${lastName?.charAt(0)}` : 'PHOTO'}
                     </div>
                </div>
                ...
                <div className="flex-grow">
                    ...
                    <p className="text-xs text-orange-600 font-bold uppercase tracking-wider mt-1 mb-4">{displayTitle}</p>
                    
                    {/* Contact Grid (UPDATED ORDER) */}
                    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-xs font-medium ${
                        darkMode ? 'text-slate-300' : 'text-slate-600'
                    }`}>
                        <a href={`tel:${displayPhone}`} className="flex items-center gap-2 hover:text-orange-500 transition-colors group">
                            <div className={`p-1 rounded ${darkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                                <Phone size={10} />
                            </div>
                            <span>{displayPhone}</span>
                        </a>
                        <a href={`mailto:${displayEmail}`} className="flex items-center gap-2 hover:text-orange-500 transition-colors group">
                            <div className={`p-1 rounded ${darkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                                <Mail size={10} />
                            </div>
                        ...
                        <a href={`https://${displayWebsite}`} className="flex items-center gap-2 hover:text-orange-500 transition-colors group">
                            <div className={`p-1 rounded ${darkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                                <Globe size={10} />
                            </div>
                ...
                <div className={`hidden sm:flex flex-col items-end pl-6 border-l ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                    ...
                    <div className="flex gap-2">
                        ...
                        {linkedinUrl && linkedinUrl.trim() !== '' && (
                            <a href={linkedinUrl} className={`w-5 h-5 flex items-center justify-center transition-transform hover:scale-110`}>
                        ...
                        {twitterUrl && twitterUrl.trim() !== '' && (
                            <a href={twitterUrl} className={`w-5 h-5 flex items-center justify-center transition-transform hover:scale-110`}>
                        ...
                        {instagramUrl && instagramUrl.trim() !== '' && (
                            <a href={instagramUrl} className={`w-5 h-5 flex items-center justify-center transition-transform hover:scale-110`}>
...
const ExoinEmailSignatures = () => {
  ...
  // Load user's preferred style if available
  useEffect(() => {
    if (user?.signatureStyle) {
      setActiveOption(user.signatureStyle === 'compact' ? 'option2' : 'option1');
    }
  }, [user]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // We rely on the backend to generate the HTML with inline styles
      // based on the user's profile and selected style.
      // This ensures the email signature works in email clients (which need inline styles)
      // while the frontend preview uses the React component (with Tailwind classes).

      await api.put('/signature/settings', {
        signatureEnabled: true,
        signatureStyle: activeOption === 'option1' ? 'executive' : 'compact',
        // We don't send signatureHtml anymore to avoid saving class-based HTML
      });
      setSaved(true);
    ...
    <div className="min-h-screen bg-slate-100 font-sans text-slate-800 flex flex-col items-center py-12 px-4">
      ...
      <div className="flex gap-4 mb-8 items-center">
        ...
        <button 
            onClick={handleSave}
            disabled={saving}
            className={`flex items-center gap-2 px-6 py-2 rounded-lg shadow-sm transition-all ${
                saved 
        ...
        >
            ...
            <span className="text-xs font-bold uppercase">{saved ? 'Saved!' : 'Save Signature'}</span>
        </button>
      </div>
        ...
        <div className="p-12 min-h-[400px] flex flex-col">
            ...
            {activeOption === 'option1' && (
                <div ref={signatureRef}>
                  <EmailSignatureTemplate data={user} darkMode={darkMode} />
                </div>
            )}
            ...
            {activeOption === 'option2' && (
                <div ref={signatureRef}>
                  <div>Option B content hidden for brevity</div>
                </div>
            )}
